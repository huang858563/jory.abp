...
using jory.abp.ToolKits.GitHub;
...
using jory.abp.Application.Caching.Authorize;

namespace jory.abp.Application.Authorize.Impl
{
    public class AuthorizeService : ServiceBase, IAuthorizeService
    {
        ...
        public async Task<ServiceResult<string>> GetLoginAddressAsync()
        {
            return await _authorizeCacheService.GetLoginAddressAsync(async () =>
            {
                ...
                var address = string.Concat(GitHubConfig.ApiAuthorize, "?client_id=", request.ClientId, "&scope=", request.Scope, "&state=", request.State, "&redirect_uri=", request.RedirectUri);

...
namespace jory.abp.BackgroundJobs.Jobs.HotNews
{
    public class HotNewsJob : IBackgroundJob
    {
        ...
        public async Task ExecuteAsync()
        {
            ...
            Task.WaitAll(listTask.ToArray<Task>());
            ...
            foreach (var list in listTask)
            {
                ...
                {
                    var nodes = ((HtmlDocument) item.Result).DocumentNode
                        .SelectNodes("//div[@class='post_item_body']/h3/a").ToList();
                    nodes.ForEach(x =>
                    {
                ...
                {
                    var nodes = ((HtmlDocument) item.Result).DocumentNode.SelectNodes("//span[@class='item_title']/a")
                        .ToList();
                    nodes.ForEach(x =>
                    {
                        hotNews.Add(new Domain.HotNews.HotNews
                ...
                {
                    var nodes = ((HtmlDocument) item.Result).DocumentNode
                        .SelectNodes("//div[@class='news__item-info clearfix']/a")
                        .Where(x => x.InnerText.IsNotNullOrEmpty()).ToList();
                    nodes.ForEach(x =>
                    {
                        hotNews.Add(new Domain.HotNews.HotNews
                        {
                ...
                {
                    var nodes = ((HtmlDocument) item.Result).DocumentNode
                        .SelectNodes("//ul[@class='news-list']/li/div[@class='txt-box']/h3/a").ToList();
                    nodes.ForEach(x =>
                    {
                        hotNews.Add(new Domain.HotNews.HotNews
                        {
                ...
                {
                    var nodes = ((HtmlDocument) item.Result).DocumentNode
                        .SelectNodes("//div[@class='channel-item']/div[@class='bd']/h3/a").ToList();
                    nodes.ForEach(x =>
                    {
                        hotNews.Add(new Domain.HotNews.HotNews
                        {
                ...
                {
                    var nodes = ((HtmlDocument) item.Result).DocumentNode
                        .SelectNodes("//div[@class='lst lst-2 hot-list']/div[1]/ul/li/a").ToList();
                    nodes.ForEach(x =>
                    {
                        hotNews.Add(new Domain.HotNews.HotNews
                        {
                ...
                {
                    var nodes = ((HtmlDocument) item.Result).DocumentNode.SelectNodes(
                            "//div[@class='hotlist-main']/div[@class='hotlist-item-toptwo']/a[2]|//div[@class='hotlist-main']/div[@class='hotlist-item-other clearfloat']/div[@class='hotlist-item-other-info']/a")
                        .ToList();
                    nodes.ForEach(x =>
                    {
                        hotNews.Add(new Domain.HotNews.HotNews
                ...
                {
                    var nodes = ((HtmlDocument) item.Result).DocumentNode.SelectNodes("//table/tbody/tr/td[2]/a")
                        .ToList();
                    nodes.ForEach(x =>
                    {
                        hotNews.Add(new Domain.HotNews.HotNews
                ...
                {
                    var nodes = ((HtmlDocument) item.Result).DocumentNode
                        .SelectNodes("//article[@class='Box-row']/h1/a").ToList();
                    nodes.ForEach(x =>
                    {
                        hotNews.Add(new Domain.HotNews.HotNews
                        {
